...
public class JavaConfig {
	...
	@Bean("myService")//id 
	public DBOracleService service(DBOracleDAO xxx) {//자동 주입. 매개 변수로 @Autowired
		DBOracleService service = new DBOracleService(xxx); //생성자를 통한 주입

		return service;
	}
	
	@Bean("myService2") // id
	public DBOracleService service2() {// 자동 주입. 매개 변수로 @Autowired
