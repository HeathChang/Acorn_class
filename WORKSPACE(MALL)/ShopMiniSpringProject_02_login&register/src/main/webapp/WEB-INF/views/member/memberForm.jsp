...
<script type="text/javascript">
	...
	$("#userid").keyup(function(e) {
				$.ajax(
				{
					type: "GET", // 1. client -> 서버로 보내지는 request방식
					url:"MemberIdCheckServlet", //2. 요청을 처리할 servlet,.jsp 지정
					dataType:"text", //응답타입 //3. 응답에 대한 데이터 형 , text ,json, xml
					data:{
						 userid: $("#userid").val() //키는 userid: 입력id
						
					},
					success: function (responseData,status,xhr) { //4-1. 정상 응답 
						$("#result").text(responseData); //responseData에 결과값 출력. 
						$("#resul").text($("#userid").val());
						//alert("성공적인 데이터 이동");
					},
					error: function(xhr,status,e) { //4-2. 문제 발생(에러 ])
						console.log("error",e);
